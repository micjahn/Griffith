...
__revision__ = '$Id: gemail.py,v 1.5 2005/09/12 22:28:02 iznogoud Exp $'
...
def send_email(self):
    if len(self.person_email):
        if self.config.get('mail_use_auth', "False") == "True":
            ...
            use_auth = 0
        mailto(self, self.config.get('mail_smtp_server', "localhost"), use_auth, self.config.get('mail_username', ""), self.config.get('mail_password', ""), self.config.get('mail_email', "griffith"), self.person_email, \
